...
│   ├── src/main/           # Electron main process, preload bridge, OS hooks
│   └── src/renderer/       # React dashboard, bubble overlay, client API wrapper
├── server/                 # FastAPI backend project
│   ├── app/                # API routers, models, services, settings, tests
│   ├── main.py             # Local uvicorn launcher
│   ├── pyproject.toml      # Backend Python project metadata
...
Root files are orchestration and documentation only; application code lives under `client` or `server`.
...
├── server/.python-version  # Backend python runtime configuration (3.12)
├── server/pyproject.toml   # Backend uv project declarations
├── server/uv.lock          # Backend dependency lock
...
- **Stopping containers**:
  ...
  This repository contains the Profile Overview page used in the desktop client.
  The high-level UI areas shown in the app are:

  - **Left Sidebar Navigation:** Primary app navigation with icons for Home,
    All Tasks, Assistant, and bottom action icons (Profile/Brain, Shop, Settings).
  - **User Hero Card:** Displays avatar, display name, email, current gamified
    rank (e.g., "Focus Apprentice"), language selection (e.g., `AR`), and active
    theme (e.g., `Calmed-Light`). The hero is seeded by the `USER` object in
    `client/src/renderer/components/Dashboard/Dashboard.jsx` and should be wired
    to the real user API when available.
  - **Gamification & Task Stats Grid:** Metric cards showing Day Streak, XP,
    Completed Tasks, Remaining Tasks, Themes Unlocked, and Sync Connections.
    These values are derived from user and task state and are used by the
    dashboard to surface rewards and sync status.

  Developers can find the icons and mock data inside
  `client/src/renderer/components/Dashboard/Dashboard.jsx` and should replace the
  mock objects with real API responses from the backend (`server/app`) when
  integrating the profile page into production flows.
